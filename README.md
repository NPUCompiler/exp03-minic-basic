...
为便于错误信息定位，在yylval中保存行号信息。

需要说明的是，yylval为bison工具定义的全局变量。在bison调用yylex函数获取下一个Token时，可通过yylval变量获取Token的属性信息，如整数值，标识符ID以及行号等信息。
...
在使用递归下降分析法时，必须确保文件满足LL(1)的文法要求。

最简单的文法改造方法：

- 左递归的产生式必须修改为右递归；
- 同一文法符号的产生式，若存在公共左因子，必须提取。
